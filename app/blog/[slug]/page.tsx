...
function PostSection({ section, index }: { section: BlogSection; index: number }) {
  return (
    ...
    >
      {section.heading && (
        <h2 className="text-[20px] font-bold text-[#1f2328] mb-3 mt-8 first:mt-0 leading-snug border-l-4 border-[#1e9b4c] pl-4">
          {section.heading}
        </h2>
      )}
      {section.subheading && (
        ...
        <ul className="flex flex-col gap-2 my-3">
          {section.list.map((item, li) => (
            <li key={li} className="flex items-start gap-2.5 text-[16px] text-[#343638] leading-relaxed">
              <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-[#1e9b4c]/10 mt-0.5">
                <Check className="w-3 h-3 text-[#1e9b4c]" />
              </span>
              <span>{item}</span>
            </li>
          ))}
    ...
    <div className="min-h-screen bg-white font-sans text-[#1f2328] flex flex-col">
      ...
      <div className="border-b border-[#e7e7e7] py-3">
        <div className="mx-auto max-w-[1240px] flex items-center gap-2 text-[14px] text-[#8b949e]">
          <Link href="/blog" className="flex items-center gap-1.5 hover:text-[#1e9b4c] transition-colors">
      ...
      <div className="flex-1 mx-4 md:mx-6 lg:mx-auto lg:max-w-[1280px] lg:px-6 py-10 md:py-14 flex flex-col lg:flex-row gap-10 lg:gap-16">
        ...
        <article className="flex-1 min-w-0">
          ...
          <div className="mt-10 rounded-xl bg-[#1b1f24] px-6 py-7 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <Link
              href="/#calculator"
              className="shrink-0 inline-flex items-center justify-center rounded-full bg-[#1e9b4c] px-6 py-2.5 text-[14px] font-semibold text-white transition hover:bg-[#17813f]"
            >
              Calculate savings
            </Link>
          </div>
          ...
          <div className="mt-10 grid grid-cols-1 sm:grid-cols-2 gap-4">
            {prev ? (
              <Link
                href={`/blog/${prev.slug}`}
                className="group flex flex-col gap-1.5 rounded-xl border border-[#e7e7e7] bg-white p-4 hover:border-[#1e9b4c]/40 hover:shadow-sm transition-all"
              >
                <span className="flex items-center gap-1.5 text-[12px] font-semibold text-[#8b949e] uppercase tracking-wider">
                  <ChevronLeft className="w-3.5 h-3.5" /> Previous
                </span>
                <span className="text-[14px] font-semibold text-[#1f2328] group-hover:text-[#1e9b4c] transition-colors line-clamp-2 leading-snug">
                  {prev.title}
                </span>
            ...
            {next ? (
              <Link
                href={`/blog/${next.slug}`}
                className="group flex flex-col gap-1.5 rounded-xl border border-[#e7e7e7] bg-white p-4 hover:border-[#1e9b4c]/40 hover:shadow-sm transition-all sm:text-right"
              >
        ...
        <aside className="lg:w-[280px] shrink-0 flex flex-col gap-6">
          ...
          <div className="rounded-xl border border-[#e7e7e7] overflow-hidden sticky top-20">
            ...
            <div className="p-4 bg-white">
              <Link
                href="/#calculator"
                className="flex items-center justify-center w-full rounded-full bg-[#1e9b4c] px-5 py-2.5 text-[14px] font-semibold text-white transition hover:bg-[#17813f]"
              >
                Calculate savings
              </Link>
            </div>
            ...
            <div className="rounded-xl border border-[#e7e7e7] bg-white overflow-hidden">
              ...
              <div className="divide-y divide-[#e7e7e7]">
                {relatedPosts.map((p) => (
                  ...
                  >
                    <span className="text-[14px] font-semibold text-[#1f2328] group-hover:text-[#1e9b4c] transition-colors line-clamp-2 leading-[22px]">
                      {p.title}
                    </span>
          ...
          <Link
            href="/blog"
            className="flex items-center justify-center gap-2 rounded-xl border border-[#e7e7e7] bg-white px-4 py-3 text-[14px] leading-[22px] font-semibold text-[#343638] hover:text-[#1e9b4c] hover:border-[#1e9b4c]/40 transition-all"
          >
