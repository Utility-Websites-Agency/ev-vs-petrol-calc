    ...
    <nav className="sticky top-0 z-50 bg-white border-b border-[#e7e7e7]">
      <div className="mx-auto max-w-[1280px] px-4 md:px-6 flex items-center justify-between h-14">
        ...
        <div className="hidden md:flex items-center gap-6">
          ...
          <Link href="/#calculator" className="px-4 py-1.5 rounded-lg text-[13px] font-semibold text-white bg-[#5e6ad2] hover:bg-[#7170ff] transition-colors">
            Calculate now
          </Link>
        </div>
      ...
      {open && (
        <div className="md:hidden border-t border-[#e7e7e7] bg-white px-4 py-3 flex flex-col gap-3">
          {NAV_LINKS.map((l) => (
            <Link key={l.href} href={l.href} className="text-[15px] text-[#1f2328] hover:text-[#5e6ad2]" onClick={() => setOpen(false)}>
              {l.label}
            </Link>
          ))}
        </div>
